...
+ (UIView *)navTitleView:(NSString *)_title
{
    ...
    lab.font = FONT(TF_BigTitle);
    lab.textColor = [UIColor whiteColor];
    lab.text = _title;
    return lab;
}

//!!!!: 通用navigation 右边按钮 有图片
/**
...
//    [btn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    ...
    [btn addTarget:_target action:selector forControlEvents:UIControlEventTouchUpInside];
    [btn setImage:[UIImage imageNamed:@"navigation_back.png"] forState:UIControlStateNormal];
    btn.imageEdgeInsets = UIEdgeInsetsMake(12.5f, 18 - 20, 12.5f, 18 + 20);
//    [btn setTitleColor:[UIColor colorWithRed:(255.0/255)23 19 green:(225.0/255) blue:(177.0/255) alpha:0.6] forState: UIControlStateHighlighted];
...
+ (UIBarButtonItem *)navButton:(id)_target action:(SEL)selector title:(NSString *)title
{
    ...
    btn.titleLabel.font = FONT(TF_Title);
    [btn addTarget:_target action:selector forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *item = [[UIBarButtonItem alloc]initWithCustomView:btn];
    return item;
}
...
+ (UIImageView *)drawLine:(CGPoint)position width:(CGFloat)width
{
    ...
    line.image = [self imageWithColor:CDLineColor andSize:CGSizeMake(1, 1)];
    return line;
...
+ (void)showPicWithUrl:(NSString *)urlStr imageView:(UIImageView *)iconImgV
{
    ...
    [iconImgV sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",kImageBaseUrl,urlStr]] placeholderImage:[Utility getPlaceHolderImage:iconImgV.frame.size string:@"img_dl_failed"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
